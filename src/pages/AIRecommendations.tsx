...
import { Sparkles, Loader2, CheckCircle2, XCircle, Send, Download, Leaf, Zap, TrendingUp, Settings, Tag, Info } from 'lucide-react';
...
export const AIRecommendations = () => {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      if (session) {
        const p = await fetchProfileFromSupabase();
        setProfile(p);
      ...
      let history = await fetchChatHistoryFromSupabase();
      if (history.length === 0) {
  ...
  const handleSend = async () => {
    ...
    try {
      const response = await chatWithEnergyAdvisor(userMsg.content, messages, profile, reports);
  ...
  if (!profile) return <div>Please set up your profile first.</div>;
